...
```{r}
idx_all_temp <- all_temp_23 %>% # one hobo temps and dates
  mutate(lagged=lag(value)-value) %>% # calc delta per 10 mins
  group_by(meta_id=meta_id) %>% # this time also group by meta id first
  group_by(day=date(tstamp)) %>% # group by day
  mutate(max_delta_day=max(abs(lagged))) %>% # get max delta of day
  ungroup() # fold out

```

Obviously these procedures produce NAs in incomplete days and do not yet yield the required mean.
...
fin_idx_all$meta_id <- as.factor(fin_idx_all$meta_id)

# test <- fin_idx_all %>% 
#   dplyr::group_by(meta_id) %>% 
#   dplyr::summarize(., avgMaxD = mean(max_delta_day, na.rm=TRUE))


# test <- fin_idx_all %>% 
#   dplyr::group_by(as.factor(meta_id)) %>% 
#   na.omit() %>% 
#   dplyr::summarise(avgDmax=mean(max_delta_day))

metaTabId <- unique(fin_idx_all$meta_id)
avgMaxDelt <- numeric(length = length(metaTabId))

for (i in 1:length(metaTabId)) {
  id_sub <- subset(fin_idx_all, meta_id==metaTabId[i])
  avgMaxDelt[i] <- mean(id_sub$max_delta_day,na.rm=T)
