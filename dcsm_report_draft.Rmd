...
```{sql connection=con}
SELECT  m.id, device_id as "HOBO id", ___ as lon, ___ as lat, full_name as term, description
FROM metadata m
join terms t on ___
join sensors s on ___
WHERE s.name='hobo' AND short='WT22'
```
...
```{sql connection=con}
with timeseries as (
  select tstamp, value from data WHERE meta_id=150 and variable_id=1   -- replace the id
),
mean_temp as (
...
union
select 'night mean' as description, idx from mean_night
union
select 'day var. coeff' as description, idx from day_coeff
```
